...
d◊|ae|mon (the "◊strong{source}") that periodically (every six hours)
...
VPS has an RSA key pair to which entropy packets are encrypted.}
...
length of the packet is written to the sink first, followed by the
encrypted packet. The sink decrypts the packet and checks its
signature, ensures the counter hasn't regressed, and verifies that the
timestamp is within an acceptable drift range. If all goes well, the
chunk of randomness is written to the system PRNG.}
...
future, I'd like to add a Fortuna source that reads data from the
...
◊ul{
  ...
  ◊li{The packets should be encrypted with NaCl.}
