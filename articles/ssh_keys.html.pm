...
◊h2{OpenSSH keys: A Walk-Through}
◊h4{2013-07-29 22:22}

◊p{I recently finished a Go package to generate and handle
◊link["http://openssh.org/"]{OpenSSH} keys. For posterity and the sake
of having notes on how to do this, I'm going to describe how the keys
are stored, and talk about converting between SSH keys and the
original key formats. The Go package can be found on
◊link["https://github.com/gokyle/sshkey"]{Github}, and I've written a
pair of programs that serve as proof-of-concepts:}

◊ul{

  ◊li{◊link["https://github.com/gokyle/sshbox"]{sshbox} allows you to
  use SSH keys to encrypt and decrypt files.}

  ◊li{◊link["https://github.com/gokyle/sshkeygen"]{sshkeygen} is a
  pure-Go implementation of the core-functionality found in
  ◊code{ssh-keygen(1)}.}

}

◊p{One of the ideas I've also been tossing around is using Github's
...
◊p{When most people think of SSH keys, they probably think of RSA if
...
◊p{◊strong{Note}: I generated a fresh set of keys for this blog
post. You can get them from
◊link["/downloads/sshkeys/demo-keys.tgz"]{here} (◊strong{currently
unavailable}).}

◊p{First, let's look at unprotected keys, as they will be the most
straight-forward. Here is our demonstration private key, found in
◊code{rsa_2048_nopass/ssh_rsa_2048}:}
...
◊p{This is a PEM-encoded RSA private key suitable for use in any
application that accepts PEM-encoded RSA private keys. The public key
(found in ◊code{rsa_2048_nopass/ssh_rsa_2048.pub}) is a different story:}
...
◊p{I've added newlines so it would fit, but this is actually all one
line. How do we make sense of this? First, let's split this line into
three elements, separated by spaces. The first element,
◊code{ssh_rsa}, tells us this is an SSH RSA key. (Actually, it
specifically refers to an SSH protocol version 2 RSA key; version 1
public keys have a different format.) The third element is a comment,
and can contain any text. By default, ◊code{ssh-keygen(1)} uses
◊code{user@hostname} as the comment. That leaves us with the middle
part; astute readers might recognise this as a base64-encoded
string. A useful tool when analysing binary data is ◊code{hexdump(1)}:
if we pass the base64-decoded middle element (which I've stored in
◊code{/tmp/key.bin}), we get}
...
◊p{Looking at the ASCII section, we see another ◊code{ssh-rsa} string,
preceded by the four bytes ◊code{0x00000007}. This is the
◊link["http://support.microsoft.com/kb/102025"]{big-endian} encoded
length of the element: "ssh-rsa" contains seven bytes. Immediately
after the "a" (or the 12th byte -- 4 bytes of length and 7 bytes of
data), we see the four bytes ◊code{0x00000003}. Let's look at next
three bytes: ◊code{0x010001}. If you've seen the low level parts of
RSA keys, you'll immediately recognise this. Otherwise, convert it to
an unsigned integer and you get ◊code{65537} -- a common RSA
...
read ◊code{0x00000101} (or 257) bytes; 257 * 8 = 2056 bytes, which is
in the range for a 2048-byte RSA key (with some of the bits going
...
◊p{Fairly straight-forward.}

◊p{With protected keys, it's a little different. The public key format
will stay the same, but the private key will be different. I've used
the password 'foo bar' in ◊code{ssh-keygen(1)} for this key, which can
be found in ◊code{rsa_2048_foobar/ssh_rsa_2048}:}
...
◊p{Right away, we can tell this isn't a vanilla PEM-encoded private
key, as there are a couple of extra headers in our certificate. The
...
initialisation vector of ◊code{E275D725D72A604BFEEFA91F4E976FBD}. How
do we get the decryption key, though? We have to use the following
◊strong{key derivation function}:}

◊ol{

   ◊li{The first eight bytes of the IV are appended to the passphrase:
   "foo bar" in hex is ◊code{666f6f20626172}, and with the first eight
   bytes of the IV, this becomes
   ◊code{666f6f20626172E275D725D72A604B}.}

   ◊li{This new byte string is then passed to MD5, which gives us the
   digest ◊code{1b407f2387eb48705dc78ed2b03a532b}. This is 16 bytes
   long, which gives us our AES key.}

   ◊li{We decrypt the body of the certificate with AES-128 in CBC mode
   with this key.}

   ◊li{The key uses the PKCS#5 padding scheme: the last byte contains
   the number of padding bytes; e.g. if there are 5 bytes of padding,
   it contains ◊code{0x05}. The last five bytes of the plaintext
   should then be ◊code{0x05} (something you should validate if you
   are decrypting the key yourself). If you decrypt the key above,
   you'll see the last eight bytes are, in fact, ◊code{0x08}.}

}
   
◊p{Here's the decrypted key (in ◊code{rsa_2048_foobar/key.bin}) with the
padding stripped:}
...
◊p{Let's base64-encode this, and split the lines at 64 characters (such as is done
with PEM).}
...
◊p{Now, let's wrap this with PEM markers:}
...
◊p{This is the unprotected RSA private key. Easy enough, right?}

◊p{Let's talk elliptic curves now.}

◊h3{Elliptic curve keys}

◊p{Elliptic curves are the new hotness (and both actually not very new
...
curve keys for the digital signature algorithm), although if you're on
a Mac, you're likely to be out of luck. Hopefully, OS X catches up on
the crypto game soon. ◊strong{Update}: I've heard reports that OS X
10.8.4 now has an updated version of OpenSSH, but I have not been able
to confirm this.}

◊p{The encrypted key format is exactly the same, so I'll focus on the
unencrypted keys.}

◊p{OpenSSH supports three curves: the NIST 256-bit, 384-bit, and 521-bit
...
◊p{That's all there is to it. As with RSA keys, this is the "vanilla" key,
...
◊p{Once again, we have our three space-delimited elements: key type, the key
...
◊p{Let's decode the key material and see what's in there:}
...
◊p{Looks pretty similar? We have three fields, as before, each preceded by
a length field. Our three fields are:}

◊ol{

   ◊li{a 19-byte field containing the text "ecdsa-sha2-nistp256" (once
   again, echoing the RSA public key material).}  ◊li{an 8-byte field
   containing "nistp256" -- the curve specifier for our EC key.}
   ◊li{a 65-byte field containing the actual EC public key. This key
   is in the format specified by
   ◊link["http://www.secg.org/download/aid-780/sec1-v2.pdf"]{SEC 1},
   the standard specifying elliptic curve cryptography.}

}

◊p{We can base64 field #3 and put it in a PEM-encoded certificate to create
...
◊h3{◊code{sshkeyconvert}}

◊p{This is a utility tool I wrote to convert SSH keys into vanilla RSA
...
◊p{I spent a lot of time reading the source for OpenSSH, and I also
found Martin Kleppmann's post
◊link["http://martin.kleppmann.com/2013/05/24/improving-security-of-ssh-private-keys.html"]{"Improving the Security of SSH Private Keys"} rather helpful.}
